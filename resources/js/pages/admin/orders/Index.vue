...
const isOrderExpanded = (row: OrderRow): boolean => expandedOrderIds.value.has(getOrderId(row));
...
const orderItems = (row: OrderRow): OrderItem[] => {
    const orderItemsValue =
        row.order_items ?? (row as Record<string, unknown>).orderItems;
...
const itemSubTotal = (item: OrderItem): number => toNumber(item.quantity) * toNumber(item.unit_price);

const onEditOrderItem = (_row: OrderRow, _item: OrderItem): void => {
    // Placeholder: collegare endpoint/azione di modifica riga ordine
};

const onRefundOrderItem = (_row: OrderRow, _item: OrderItem): void => {
    // Placeholder: collegare endpoint/azione di rimborso riga ordine
};

const onDeleteOrderItem = (_row: OrderRow, _item: OrderItem): void => {
    // Placeholder: collegare endpoint/azione di cancellazione riga ordine
};
...
const orderItemsHeaders = ['Tipologia', 'Quantità', 'Prezzo unitario', 'Totale riga', 'Azioni'];
</script>
    ...
    >
        <template #row="slotProps">
            <tr
                ...
                @click="hasOrderChildren(asRecord(slotProps.row)) && toggleOrderExpanded(asRecord(slotProps.row))"
            >
                <td
                ...
                >
                    {{ formatCellValue(getNestedValue(asRecord(slotProps.row), column.field_name), column) }}
                </td>
            </tr>
            <tr
                v-if="isOrderExpanded(asRecord(slotProps.row)) && hasOrderChildren(asRecord(slotProps.row))"
            >
                <td
                    :colspan="
                        slotProps.rowColspan ?? (slotProps.columns.length + (slotProps.hasActions ? 1 : 0))
                    "
                    class="bg-muted/40"
                >
                    <div class="px-6 py-3">
                                        ...
                                        <td class="px-3 py-2 border-r border-border">
                                            <div class="flex items-center gap-2">
                                                <button
                                                    ...
                                                    @click.stop="onEditOrderItem(asRecord(slotProps.row), item)"
                                                    title="Modifica"
                                                ...
                                                <button
                                                    ...
                                                    @click.stop="onRefundOrderItem(asRecord(slotProps.row), item)"
                                                    title="Rimborso"
                                                ...
                                                <button
                                                    ...
                                                    @click.stop="onDeleteOrderItem(asRecord(slotProps.row), item)"
                                                    title="Elimina"
