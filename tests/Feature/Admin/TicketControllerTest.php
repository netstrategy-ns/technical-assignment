...
class TicketControllerTest extends AdminControllersTestCase
{
    ...
    public function test_index_route_renders_expected_inertia_payload(): void
    {
        ...
        $response->assertInertia(function (Assert $page) {
            $page->component('admin/tickets/Index')
                ->has('resource')
    ...
    {
        $request = $this->makeInertiaRequest('/admin/tickets');
        $response = (new TicketController())->index($request);

        $payload = $this->extractInertiaPayload($response, $request);
        $this->assertInertiaPayloadIndexShape($payload, 'admin/tickets/Index', 'tickets');
